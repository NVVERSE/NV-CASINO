...
ADMINS = [7995159553]  # ✅ Yahan bracket lagakar ID sahi kar di hai
...
async def check_user_joined(user_id: int) -> bool:
    try:
        # Note: Bot check tabhi karega jab woh khud us group me admin hoga.
        ...
        # Agar bot admin nahi hai ya check nahi kar pa raha, toh default false dega taaki user join kare
        return False
...
async def check_join_again(callback: CallbackQuery):
    ...
    if not is_joined and user_id not in ADMINS:
        await callback.answer("❌ Aapne abhi tak group join nahi kiya hai!", show_alert=True)
        return
